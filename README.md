...
| column | type | notes |
|---|---|---|
| `source` | string | `faostat` \| `faostat_product` \| `owid` \| `wb_wdi` \| `india_dof_consumption` \| `india_dof_state_consumption` \| `india_dof_district_npk` |
| `source_record_id` | string | source + iso3 + nutrient + year (+ state/district if sub-national) |
| `country_iso3` | string | |
| `country_name` | string | |
| `state_or_region` | string | Sub-national name (Indian state/UT, district, ...). NULL for national rows. |
| `year` | int32 | |
...
|---|---|---|
| **India FAI Yearbook (PDF)** | **DROPPED** — no PDF scrape | Replaced with structured `data.gov.in` sources (the original CCS path is geo-blocked from US: `eands.dacnet.nic.in` DNS-fails, `eands.da.gov.in` returns 403 from Google Cloud's geo-fence). Schema probe of the CCS data.gov.in mirror UUIDs (wheat 2016-17, paddy 2015-16, multi-crop 2015-21) confirmed those resources carry only monetary "Fertilisers and Manure" Rs/ha — **NOT kg/ha** — so they're dead-ends for fertilizer-allocation purposes. Three usable data.gov.in fertilizer sources are now wired in `scripts/refresh.py`: (a) `india_dof_subsidy` annual subsidy by product 2002-2011 (UUID `2e0e6c04...`); (b) `india_dof_state_consumption` state-year-product CONSUMPTION 2019-20→2023-24 (UUID `c7c7d147...`, 37 states × 4 products × 5 years, N/P2O5/K2O derived via stoichiometry); (c) `india_dof_district_npk` direct N/P2O5/K2O tonnes by district 2019-20 (UUID `93051de3...`, 30 districts, no derivation). All use `state_or_region` column for sub-national granularity. Note: the data.gov.in guest API key silently caps each call at 10 records — pullers paginate via `offset`. **ICRISAT DLD is NOT a substitute** — district aggregate, not per-crop. |
