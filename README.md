...
|---|---|---|
| **India FAI Yearbook (PDF)** | **DROPPED** — no PDF scrape | Replace with structured sources: (1) `desagri.gov.in` Table 14.4(a) state-wise N/P/K consumption (PDF + Excel, annual, **geo-blocked from US** → needs India-resident runner or VPN); (2) `data.gov.in` Dept-of-Fertilizers via REST + public guest API key — **annual subsidy + annual all-India consumption now wired** in puller as of 2026-05-22 evening; (3) Cost of Cultivation Survey (CCS) via `eands.dacnet.nic.in` (PRIMARY) is **geo-blocked from US** — DNS fails, alternate host `eands.da.gov.in` returns 403 from Google Cloud's geo-fence. The CCS data is mirrored on `data.gov.in` under per-crop resource UUIDs (see below) — same REST path as DoF subsidy/consumption, no geo-block. Candidate UUIDs discovered 2026-05-23 but unvalidated due to shared-guest-key rate-limit at discovery time: `2f099ca8-2401-4cca-bab9-a8b0e3d5a4a6` (wheat 2016-17), `b3619fa8-f93f-43f6-ae2b-e0d2e76b3d15` (paddy 2015-16), `e94ed140-4de8-421b-a3ae-123156a884ff` (paddy+wheat+maize+jute+pulses+cotton 2015-16 to 2020-21 GVO + A2 cost). Wire after schema inspection confirms fertilizer kg/ha columns. **ICRISAT DLD was originally listed here but is incorrect** — investigated 2026-05-22 evening, ICRISAT publishes aggregate seasonal N/P/K by district, **NOT per-crop**. |
